...
Consider the fast rotation signal of the previous subsection above; call it $S_0(t)$ for now. We have that given an initial symmetric longitudinal distribution $\xi(t')$, \[S(t)=\int\xi(t')S_0(t-t')dt'=(\xi\ast S_0)(t)\] If the support of $\xi$ is $(t_1,t_2)$, then the support of $S(t)$ is $(t_0+t_1,\infty)$ (the support of $S_0(t)$ is $(t_0,\infty)$). So then we may just apply the reasoning from the previous section to see that \[\hat{S}(\omega)=\sqrt{\frac{2}{\pi}}\int^{\infty}_{t_0+t_1}S(t)\cos\omega(t-t_0-t_1)dt\] and \[S(t)=\sqrt{\frac{2}{\pi}}\int^{\infty}_{0}\hat{S}(\omega)\cos\omega(t-t_0-t_1)d\omega\] This is a valid formula if $t_0+t_1\geq0$. In the case that $t_1+t_0\leq0$ or $t_1=-\infty$ there will be a problem, as the muons are injected at $t=0$. But assuming $\xi(t')$ is sufficiently narrow, starting at $t=0$ will not miss much of the signal, making the deformation to the spectrum negligible. Hence in this case we have that \[\hat{S}(\omega)=\sqrt{\frac{2}{\pi}}\int^{\infty}_0S(t)\cos\omega tdt\] and \[S(t)=\sqrt{\frac{2}{\pi}}\int^{\infty}_{0}\hat{S}(\omega)\cos\omega td\omega\] We note however that this time \[S(t)=\sum^{\infty}_{n=0}\frac{\rho\left(\frac{t}{(n+\theta/2\pi)T}-1\right)}{(n+\theta/2\pi)T}\]%Using $(4)$, we have \[S(t)=\frac{2}{\pi}\int\xi(t')dt'\int^{\infty}_0\cos\omega(t-t'-t_0)d\omega\int^{\infty}_{t_0}S_0(\tilde{t})\cos\omega(\tilde{t}-t_0)d\tilde{t}=\]\[\sqrt{\frac{2}{\pi}}\int\xi(t')dt'\int^{\infty}_0\hat{S}_0(\omega)\cos\omega(t-t'-t_0)d\omega=\]\[\sqrt{\frac{2}{\pi}}\int\xi(t')dt'\int^{\infty}_0\hat{S}_0(\omega)(\cos\omega(t-t_0)\cos\omega t'+\sin\omega(t-t_0)\sin\omega t')d\omega=\]\[\sqrt{\frac{2}{\pi}}\int\xi(t')dt'\int^{\infty}_0\hat{S}_0(\omega)\cos\omega(t-t_0)\cos\omega t'd\omega=\]\[\sqrt{\frac{2}{\pi}}\int^{\infty}_0\hat{S}_0(\omega)\langle\cos\omega t'\rangle\cos\omega(t-t_0)d\omega\] where we used parity of sine and $\langle\cos\omega t'\rangle=\int\xi(t')\cos\omega t'dt'$. So we have that% 

\subsection{Corrections to the Fourier Transform}

If the detector detects the muons at a time $t_s>t_0$, then the resulting Fourier transform of the observed fast rotation signal has a missing component \[\Delta(\omega)=\sqrt{\frac{2}{\pi}}\int^{t_s}_{t_0}S(t)\cos\omega(t-t_0)dt\] while the immediately observed frequency spectrum is \[\hat{S}(\omega)=\sqrt{\frac{2}{\pi}}\int^{\infty}_{t_s}S(t)\cos\omega(t-t_0)dt\] However, we do not observe $S(t)$ on $(t_0,t_s)$, so we need to find a way to correct the frequency spectrum. 

In order to do this, we first note that the Fourier transform has associated with it an uncertainty principle, namely that $\Delta f\Delta t\sim1$. If $t_s$ isn't too great, we can then expect that the spread of $\Delta(\omega)$ about the magic frequency will be large while the spread of $\hat{S}(\omega)$ will change negligibly. So we may express $S(t)$ using the observed $\hat{S}(\omega)$ through formula $(6)$. Thus we have that \[\Delta(\omega)=\frac{2}{\pi}\int^{t_s}_{t_0}\int^{\infty}_0\hat{S}(\omega')\cos\omega'(t-t_0)\cos\omega(t-t_0)d\omega' dt\] Integrating with respect to $t$ first, we get \[\Delta(\omega)=\frac{1}{\pi}\int^{\infty}_0\hat{S}(\omega)\left(\frac{\sin(\omega-\omega')(t_s-t_0)}{\omega-\omega'}+\frac{\sin(\omega+\omega')(t_s-t_0)}{\omega+\omega'}\right)d\omega'\] We may neglect the second term in the parentheses, so \[\Delta(\omega)=\frac{1}{\pi}\int^{\infty}_0\hat{S}(\omega)\frac{\sin(\omega-\omega')(t_s-t_0)}{\omega-\omega'}d\omega'\]
