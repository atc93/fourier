...
We call the fast rotation signal with no inital bunch length $S_0(t)$. Given an initial longitudinal distribution $\xi(t')$, the fast rotation signal is described by \[S(t)=\int\xi(t')S_0(t-t')dt'=(\xi\ast S_0)(t)\] In the ideal case we would directly apply the convolution theorem to find that 

\begin{equation}
\hat{S}(\omega)=\hat{\xi}(\omega)\hat{S}_0(\omega)
...
\end{equation} suggesting that $\hat{S}(\omega)=\hat{S}_0(\omega)\langle\cos\omega t'\rangle$. We can see that this is true by looking back at$~\eqref{eq:ConvoSpectrum}$: \[\hat{\xi}(\omega)=\int^{\infty}_{-\infty}\xi(t')e^{i\omega t'}dt'=\int^{\infty}_{-\infty}\xi(t')\cos\omega t'dt'=\langle\cos\omega t'\rangle\] and therefore $\hat{S}(\omega)=\hat{\xi}(\omega)\hat{S}_0(\omega)=\hat{S}_0(\omega)\langle\cos\omega t'\rangle$. Thus $~\eqref{eq:EvenBunchFT}$ is the inverse Fourier transform, implying that \[\hat{S}(\omega)=\int^{\infty}_{t_0}S(t)\cos\omega(t-t_0)dt\] 
